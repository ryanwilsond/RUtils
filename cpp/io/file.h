...
public:
    ...
    _NODISCARD bool Exists(const std::string& path) const noexcept {
    ...
    /// Attempts to read all text from file, throws if the file does not exist
    /// @param path path of file
    /// @returns all text from file (including line endings)
    _NODISCARD std::string ReadAllText(const std::string& path) const noexcept {
        std::string buffer;
        ...
        if (stream.is_open()) {
            while (std::getline(stream, bufferLine)) {
                buffer += bufferLine;
                buffer += "\n";
            }
